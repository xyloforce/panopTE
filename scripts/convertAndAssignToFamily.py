...
#bin Score   %_Div   %_Del   %_Ins   Query   Beg.    End.    Length  Sense   Element Family  Pos_Repeat_Beg  Pos_Repeat_End  Pos_Repeat_Left ID
...
for line in inputHandler:
    ...
    if line[10] not in families: # repName is the key
        families[line[10]] = [line[11] + '/' + line[12], 0, 0] # initialize counters + merge repClass_repFamily
...
for index in range(0, len(file_content)):
    file_content[index][4] = str(float(file_content[index][4]) / maxScore * 1000)
    file_content[index] = "\t".join(file_content[index]) + "\n"

output.writelines(file_content)

for value in families:
    familiesTE.write(value + "\t" + families[value][0] + "\t" + str(families[value][1]) + "\t" + str(families[value][2] / families[value][1]) + "\n")
